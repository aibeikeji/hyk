...
$url=substr($str,0,$n);
	...
	if($czorder['state']==1){
		...
		///////////加积分//////////////
         ...
         if(!$czorder['store_id']){
         	$level=pdo_get('zhvip_level',array('uniacid'=>$czorder['uniacid']),array(),'','level ASC');
			  $data2['grade']=$level['id'];
	...
	if($timeorder['state']==1){
		...
		if($user['grade']>0){
			...
			$level=pdo_get('zhvip_level',array('uniacid'=>$timeorder['uniacid']),array(),'','level ASC');
			  $data2['grade']=$level['id'];
			  $data2['code_time']=time(); 
