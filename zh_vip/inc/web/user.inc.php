...
$pagesize = 10;
$sql = "select a.*,b.name as level_name  from " . tablename("zhvip_user") . "a" . " left join " . tablename("zhvip_level") . " b on b.id=a.grade WHERE  (a.nickname LIKE :name || a.name LIKE :name || a.tel LIKE :name || a.vip_number LIKE :name)  and a.uniacid=:uniacid and a.grade>0 order by id desc";
$select_sql = $sql . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize;
$list = pdo_fetchall($select_sql, array(':uniacid' => $_W['uniacid'], ':name' => $where));
$total = pdo_fetchcolumn("select count(*)  from " . tablename("zhvip_user") . "a" . " left join " . tablename("zhvip_level") . " b on b.id=a.grade WHERE  (a.nickname LIKE :name || a.name LIKE :name || a.tel LIKE :name || a.vip_number LIKE :name)  and a.uniacid=:uniacid and a.grade>0", array(':uniacid' => $_W['uniacid'], ':name' => $where));
