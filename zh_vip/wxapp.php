...
//url(七牛)
	...
	public function doPageAddVip(){
		global $_GPC, $_W;
		$res=pdo_get('zhvip_level',array('uniacid'=>$_W['uniacid']),array(),'','level ASC');
		$data['grade']=$res['id'];
		$data['code_time']=time(); 
...
public function doPageStore(){
	...
	if($system['md_xs']==1){
		...
		$sql="SELECT *, ROUND(6378.138*2*ASIN(SQRT(POW(SIN(($lat*PI()/180-SUBSTRING_INDEX(coordinates, ',', 1)*PI()/180)/2),2)+COS($lat*PI()/180)*COS(SUBSTRING_INDEX(coordinates, ',', 1)*PI()/180)*POW(SIN(($lng*PI()/180-SUBSTRING_INDEX(coordinates, ',', -1)*PI()/180)/2),2)))*1000) AS juli  FROM ".tablename("zhvip_store") ." where uniacid=".$_W['uniacid']." ORDER BY juli ASC";
		$res=pdo_fetch($sql);
		echo json_encode($res);
	}	
}
